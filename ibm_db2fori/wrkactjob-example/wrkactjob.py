...
conn = dbi.connect('DSN=DSN_Db2iBKP') #DSN IBM i Access Driver
...
def root():
    ...
    query = "select %s from table(qsys2.active_job_info(RESET_STATISTICS => ?)) x limit 20" % column_string
...
run(host='172.17.50.203', port=3333, debug=True, reloader=True)
